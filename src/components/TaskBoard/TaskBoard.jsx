...
const isDeadlineClose = (isoDate, status) => {
  if (status === 'done') return false;
  const timeRemaining = new Date(isoDate).getTime() - Date.now();
  return timeRemaining > 0 && timeRemaining < 172800000;
};

// Memoized TaskCard — prevents re-render unless its own props change
const TaskCard = memo(function TaskCard({ task, onMove, isFirst, isLast }) {
  const isUrgent = isDeadlineClose(task.dueDate, task.status);

  return (
    <article
      aria-label={`Task: ${task.title}`}
      className={`bg-white dark:bg-valhalla-slate border rounded-xl p-4 transition-all duration-200 hover:-translate-y-1 hover:shadow-md
        ${isUrgent
    ...
    >
      <div className="flex justify-between items-center mb-2">
        <div className="flex gap-2 items-center">
          <span
            ...
            className="text-[0.65rem] font-bold bg-gray-100 dark:bg-valhalla-void px-1.5 py-0.5 rounded text-gray-500 dark:text-gray-400"
          >
            {task.priority}
          </span>
          {isUrgent && (
            <span role="alert" aria-live="assertive" className="text-[0.65rem] font-bold text-red-500 flex items-center gap-1 animate-pulse">
              ⚠️ DUE SOON
            </span>
          )}
        </div>
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-2 text-xs text-valhalla-ink/60 dark:text-gray-400">
          <div
            aria-hidden="true"
            className="w-6 h-6 rounded-full bg-gradient-to-br from-valhalla-gold to-amber-700 dark:from-valhalla-neon dark:to-purple-500 text-white flex items-center justify-center font-bold text-[0.6rem]"
          >
            {task.owner.charAt(0)}
          </div>
          <span aria-label={`Owner: ${task.owner}`}>{task.owner}</span>
        </div>

        <div className="flex gap-1" role="group" aria-label="Move task">
          {!isFirst && (
            <button
              onClick={() => onMove(task.id, 'prev')}
              aria-label={`Move task "${task.title}" to previous column`}
              className="w-6 h-6 rounded flex items-center justify-center border border-valhalla-ink/10 dark:border-white/10 text-valhalla-ink/50 dark:text-gray-400 hover:bg-valhalla-ink/5 dark:hover:bg-white/5"
            >
              ←
            </button>
          )}
          {!isLast && (
            <button
              ...
              aria-label={`Move task "${task.title}" to next column`}
              className="w-6 h-6 rounded flex items-center justify-center border border-valhalla-ink/10 dark:border-white/10 text-valhalla-ink/50 dark:text-gray-400 hover:bg-valhalla-ink/5 dark:hover:bg-white/5"
            >
              →
            </button>
...
export default function TaskBoard() {
  ...
  // Memoize column→task mapping to prevent recalculation on every render
  ...
  return (
    <section aria-label="Task Board" className="flex flex-col h-full gap-6">
      <header className="flex justify-between items-center">
        ...
        <form onSubmit={handleAddTask} aria-label="Add new task" className="flex gap-2">
          ...
          <button
            type="submit"
            aria-label="Add task"
            className="bg-valhalla-ink dark:bg-valhalla-neon text-white dark:text-valhalla-void px-4 py-2 rounded-lg font-bold hover:opacity-90 transition-opacity"
          >
            Add
          </button>
        </form>
      ...
      <div role="list" aria-label="Kanban columns" className="grid grid-cols-4 gap-6 flex-1 min-h-0">
        {COLUMNS.map((column, colIdx) => (
          <div
            key={column.id}
            role="listitem"
            aria-label={`${column.title} column, ${tasksByColumn[column.id].length} tasks`}
            className="flex flex-col rounded-2xl p-4 h-full max-h-full glass"
          >
            <div className="flex justify-between items-center mb-4 pb-2 border-b border-valhalla-ink/10 dark:border-white/10">
              <h3 className="text-sm font-bold uppercase tracking-wider text-valhalla-ink/50 dark:text-gray-400">
                {column.title}
              </h3>
              <span
                aria-label={`${tasksByColumn[column.id].length} tasks`}
                className="bg-white/50 dark:bg-white/10 text-valhalla-ink dark:text-gray-300 text-xs px-2 py-0.5 rounded-full font-bold"
              >
                {tasksByColumn[column.id].length}
              </span>
            </div>

            <ul aria-label={`Tasks in ${column.title}`} className="flex flex-col gap-4 overflow-y-auto pr-1 pb-4">
